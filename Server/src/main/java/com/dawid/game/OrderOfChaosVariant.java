...
public class OrderOfChaosVariant implements GameVariant {
    public void initializeGame(Lobby lobby) {
        ...
        for(Field field: playerHomeFields) {
            ...
            lobby.notifyAll("Moved: Player MOVE " + field.getHome() + " " + board.getCoordinates(field) + " " + board.getCoordinates(field2));
            System.out.println("Moved: Player MOVE " + field.getHome() + " " + board.getCoordinates(field) + " " + board.getCoordinates(field2));
